...
module.exports.verificationPremiumTextAndTitle=(title,text,users,users_accounts,accounts)=>{
    if(text.length>accounts[0][0].text_limit){
      return{message:"Your text is too long."}
    }
    if(title.length>100){
        return {message:"The news title is too long"}
    }
    const today = moment();
...
module.exports.verificationAuthentificatedTextAndTitle=(title,text,users,users_accounts,accounts)=>{
    if(accounts[0][0].max_attempts<users_accounts[0][0].attempts){
      return {message:"Your free trials expired. Please buy a premium account."}
    }
    if(title.length>100){
        return {message:"The news title is too long"}
    }
