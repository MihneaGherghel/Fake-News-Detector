const { verificationAuthentificatedTextAndTitle, verificationPremiumTextAndTitle, verificationURL } = require('./utilVerifUser');
const db=require('../../data/database');

module.exports.fakeNewsAuthenticatedTextandTitle=async (title,text,email)=>{
    ...
    if(users_accounts[0][0].account_id==3){
      result=verificationAuthentificatedTextAndTitle(title,text,users,users_accounts,accounts)
    }
    else{
      result=verificationPremiumTextAndTitle(title,text,users,users_accounts,accounts)
    }
    return result
}
